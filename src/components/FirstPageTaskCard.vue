<template>
  <div :class="['card', { 'expanded': isExpanded, 'editing': isEditing, 'highlighted': isHighlighted }]" @click="toggleHighlight">
      <div v-if="isExpanded" :class="['card-header', statusColor]">
          <div class="status">{{ task.status }}</div>
          <button class="close-btn" @click.stop="handleClose">X</button>
      </div>
      <div class="card-body">
...
export default {
  ...
  data() {
      return {
          isExpanded: false,
          isEditing: false,
          isHighlighted: false, // New property for highlighting
  ...
  methods: {
      ...
      getDescription(action) {
          if (action === 'status changed') {
              ...
              return `Updated task: ${this.editedTask.name} with priority ${this.editedTask.priority}, story point ${this.editedTask.storyPoint}`;
      ...
      toggleHighlight() {
          if (!this.isExpanded) {
              this.isHighlighted = !this.isHighlighted; // Toggle highlight only when not expanded
          }
      },
      expandCard() {
          this.isExpanded = true;
          this.$emit('task-expanded', this.task);
      },
      handleClose() {
          if (this.isEditing) {
...
.highlighted {
  border: 3px solid rgb(126, 197, 255); /* Highlight effect */
  background-color: #208fda; /* Lighter background */
  }
.card {
  left: 10px;
  top: 20px;
  ...
  margin: 10px;
  transition: background-color 0.01s ease-in-out, border 0.01s ease-in-out;
}
...
.card.expanded {
  ...
  width: 835px;
  height: 570px; /* Default height when expanded */
  z-index: 1000;
...
.expand-btn {
  ...
  border-color: #aaa; /* Lighter border color */
