<template>
  <div :class="['card', { 'expanded': isExpanded, 'editing': isEditing }]">
    ...
    <div class="card-body" @click="expandCard">
      ...
      <div v-if="isExpanded" class="expanded-info">
        <div v-if="!isEditing">
          ...
          <button class="history-button" @click="toggleHistoryModal">View History</button>
        </div>
    ...
    <div v-if="isHistoryModalOpen" class="modal-overlay" @click="toggleHistoryModal">
...
<script>
import { db, historyRef } from "@/firebase/init.js";
...
export default {
  ...
  methods: {
    async addToHistory(action) {
      let details = this.getDescription(action);
      if (details) {
        const historyEntry = {
          action: action,
          details: details,
          timestamp: new Date().toLocaleString(),
          taskId: this.task.id, // Add task ID for reference
        };

        // Push to local history for UI updates
    ...
    async fetchHistory() {
      const q = query(historyRef, where("taskId", "==", this.task.id));
    ...
    viewHistory() {
      // Handle the logic for viewing the history log
      const historyMessages = this.history.join('\n'); // Join the history messages with newlines
      alert(historyMessages || 'No history available.'); // Display the messages in an alert
      },
    ...
    getDescription(action) {
      ...
      if (action === 'task updated') {
        // Compare task name
        if (this.task.name !== this.editedTask.name) {
          changes.push(`Task name: ${this.task.name} has been changed to ${this.editedTask.name}`);
        }
        
        // Compare priority
        if (this.task.priority !== this.editedTask.priority) {
          changes.push(`Priority: ${this.task.priority} has been changed to ${this.editedTask.priority}`);
        }
        
        // Compare story point
        if (this.task.storyPoint !== this.editedTask.storyPoint) {
          changes.push(`Story point: ${this.task.storyPoint} has been changed to ${this.editedTask.storyPoint}`);
        }
        
        // Compare tags
        if (JSON.stringify(this.task.tags) !== JSON.stringify(this.editedTask.tags)) {
          changes.push(`Tags: ${this.task.tags.join(', ')} has been changed to ${this.editedTask.tags.join(', ')}`);
        }
        
        // Compare other fields as needed (like assignee, stages, type)
        if (this.task.type !== this.editedTask.type) {
          changes.push(`Task type: ${this.task.type} has been changed to ${this.editedTask.type}`);
        }
        if (this.task.stages !== this.editedTask.stages) {
          changes.push(`Stages: ${this.task.stages} has been changed to ${this.editedTask.stages}`);
        }
...
.card.editing {
  margin-top: 40px;
  height: auto; /* Height when editing */
  overflow-y: auto;
}
.header-default {
...
.modal-content {
  background-color: #ffffff; /* Keep the modal background white for contrast */
...
.modal-content h2 {
  color: #111111; /* Make the heading even darker */
}

.modal-content ul li {
  color: #333333; /* Darker text for the history log items */
  font-size: 14px;
  margin: 5px 0;
}
